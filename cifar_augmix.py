...
Supports WideResNet, DenseNet, ResNeXt models on CIFAR-10 and CIFAR-100 as well
as evaluation on CIFAR-10-C and CIFAR-100-C.
Four types of input space: RGB, one-hot encoding, thermometer encoding, and P2BE.

The code is based on the original augmix implementation: https://github.com/google-research/augmix
"""
...
parser.add_argument(
    '--corrupted',
    type=str,
    default='./data/cifar-c/CIFAR-10-C/',
    help='Path to CIFAR-10-C or CIFAR-100-C')
parser.add_argument(
    '--model',
    '-m',
    type=str,
...
def train(net, train_loader, optimizer, scheduler, model_name=args.model, optimizer_e=None, scheduler_e=None):
  """Train for one epoch."""
  net.train()
  loss_ema = 0.
...
def main():
  ...
  if args.model == 'densenet':
    ...
    print('{} m: {}, lambda: {}'.format(args.model, args.m, args.coefficient_smooth))
    net = WideResNetB(args.layers, num_classes, args.widen_factor, args.droprate, m=args.m, beta=args.coefficient_smooth)
    ...
    if args.fix == '':
      optimizer_e = torch.optim.AdamW([{'params': net.embedding_parameters()}],
        ...
        if os.path.isfile(args.fix):
          print('loading p2be in {}'.format(args.fix))
          checkpoint = torch.load(args.fix)
          net.p2be.embedding.data = checkpoint['state_dict']['module.p2be.embedding'].data.cpu()
          optimizer_e = None
  ...
  for epoch in range(start_epoch, args.epochs):
    ...
    train_loss_ema = train(net, train_loader, optimizer, scheduler, model_name=args.model, optimizer_e=optimizer_e, scheduler_e=scheduler_e)
    test_loss, test_acc = test(net, test_loader)
