...
class DenseNet(nn.Module):
  ...
  def __init__(self, growth_rate, depth, reduction, n_classes, bottleneck, m=32, beta=-1.0e-1):
    super(DenseNet, self).__init__()

    self.m, self.beta = m, beta
    # p2be
    self.p2be = P2BE(m=m, beta=beta)
...
def densenetb(growth_rate=12, depth=40, num_classes=10, m=64, beta=1.0e-1):
  model = DenseNet(growth_rate, depth, 1., num_classes, False, m=m, beta=beta)
  return model
