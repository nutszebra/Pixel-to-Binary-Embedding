...
class DenseNetThermometer(nn.Module):
  """DenseNet main class."""

  def __init__(self, growth_rate, depth, reduction, n_classes, bottleneck, m=32):
    super(DenseNetThermometer, self).__init__()

    self.thermometer = Thermometer(m=m)
    ...
    self.conv1 = nn.Conv2d(int(3 * m), n_channels, kernel_size=3, padding=1, bias=False)
...
def densenet_thermometer(growth_rate=24, depth=40, num_classes=10, m=32):
  model = DenseNetThermometer(growth_rate, depth, 1., num_classes, False, m=m)
  return model
