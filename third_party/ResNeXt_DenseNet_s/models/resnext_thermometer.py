...
class CifarResNeXtThermometer(nn.Module):
  """ResNext optimized for the Cifar dataset, as specified in https://arxiv.org/pdf/1611.05431.pdf."""

  def __init__(self, block, depth, cardinality, base_width, num_classes, m=32):
    super(CifarResNeXtThermometer, self).__init__()
    ...
    self.conv_1_3x3 = nn.Conv2d(int(3 * m), 64, 3, 1, 1, bias=False)
...
def resnext29_thermometer(num_classes=10, cardinality=4, base_width=32, m=32):
  model = CifarResNeXtThermometer(ResNeXtBottleneck, 29, cardinality, base_width,
                       num_classes, m=m)
  return model
